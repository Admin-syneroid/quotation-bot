...
>
> **Public vs private:** a **public** repo gets *unlimited* free Actions minutes → keep the 10-minute schedule. A **private** repo only gets 2,000 free minutes/month → change the cron in `.github/workflows/quotations.yml` to `"0 * * * *"` (hourly) to stay free. The repo holds only code — no secrets, no client data — so public is safe and recommended.
